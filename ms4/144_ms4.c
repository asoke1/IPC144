...
struct Item itemEntry(int sku)
{
	...
	flushKeyboard();
	printf("      Price: ");
	temp.price = getDoubleLimited(0.01, 1000.00);//getDouble();
	printf("   Quantity: ");
	temp.quantity = getIntLimited(1, MAX_QTY); //getInt();
	printf("Minimum Qty: ");
	temp.minQuantity = getIntLimited(0, 100); //getInt();
	printf("   Is Taxed: ");
	temp.isTaxed = yes();
...
{
	int read;
	read = fscanf(dataFile, "%d,%d,%d,%lf,%d,%20[^\n]", &item->sku, &item->quantity, &item->minQuantity, &item->price, &item->isTaxed, item->name);
	if (read == 6) {
		return 1;
	}
